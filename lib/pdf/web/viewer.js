...
const PDFViewerApplication = {
  ...
  progress(level) {
    ...
    if (percent <= this.loadingbar.percent) { return; } this.loadingbar.percent="percent;" if (this.pdfdocument?.loadingparams.disableautofetch ?? _app_options.appoptions.get("disableautofetch")) this.loadingbar.setdisableautofetch(); }, load(pdfdocument) this.pdfdocument="pdfDocument;" pdfdocument.getdownloadinfo().then(({ length })> {
...
function normalize(text) {
  ...
  normalized = normalized.replace(normalizationRegex, (match, p1, p2, p3, p4, p5, p6, p7, p8, i) => {
    ...
    if (p4) {
      ...
      for (let j = 1; j <= 1 jj; j++) { positions.push([i - shift + j, j]); } shiftorigin if (hastrailingdasheol) i 1; 1, shift]); eol return p4.slice(0, len); p4; (p5) const len="p5.length" 2; len, p5.slice(0, -2); (p6) p6.slice(0, -1); (p7) 1]); " "; (i syllablepositions[syllableindex]?.[1]) newcharlen="syllablePositions[syllableIndex][0]" ++syllableindex; for (let j="1;" <="newCharLen;" (shift j), p8; }); positions.push([normalized.length, [normalized, positions, hasdiacritics]; function getoriginalindex(diffs, pos, len) (!diffs) [pos, len]; start="pos;" end="pos" let _ui_utils.binarysearchfirstitem)(diffs, x> x[0] >= start);
  if (diffs[i][0] > start) {
    --i;
  }
  ...
  #shouldDirtyMatch(state) {
    ...
    switch (state.type) {
      case "again":
        ...
        return pageNumber >= 1 && pageNumber <= linkservice.pagescount && pagenumber !="=" linkservice.page !(this.onispagevisible?.(pagenumber) ?? true); case "highlightallchange": return false; } true; #isentireword(content, startidx, length) { let match="content.slice(0," startidx).match(not_diacritic_from_end_reg_exp); if (match) const first="content.charCodeAt(startIdx);" limit="match[1].charCodeAt(0);" ((0, _pdf_find_utils.getcharactertype)(first)="==" (0, _pdf_find_utils.getcharactertype)(limit)) + length).match(not_diacritic_from_start_reg_exp); last="content.charCodeAt(startIdx" length - 1); _pdf_find_utils.getcharactertype)(last)="==" #calculateregexpmatch(query, entireword, pageindex, pagecontent) matches="this._pageMatches[pageIndex]" = []; matcheslength="this._pageMatchesLength[pageIndex]" (!query) return; diffs="this._pageDiffs[pageIndex];" match; while ((match="query.exec(pageContent))" null) (entireword !this.#isentireword(pagecontent, match.index, match[0].length)) continue; [matchpos, matchlen]="getOriginalIndex(diffs," match[0].length); (matchlen) matches.push(matchpos); matcheslength.push(matchlen); #converttoregexpstring(query, hasdiacritics) matchdiacritics isunicode="false;" query="query.replaceAll(SPECIAL_CHARS_REG_EXP," (match, p1, p2, p3, p4, p5)> {
      if (p1) {
        return `[ ]*\\${p1}[ ]*`;
      }
      if (p2) {
...
class PDFViewer {
  ...
  async getAllText() {
    const texts = [];
    const buffer = [];
    for (let pageNum = 1, pagesCount = this.pdfDocument.numPages; pageNum <= pagescount; ++pagenum) { if (this.#interruptcopycondition) return null; } buffer.length="0;" const page="await" this.pdfdocument.getpage(pagenum); items page.gettextcontent(); for (const item of items) (item.str) buffer.push(item.str); (item.haseol) buffer.push("\n"); texts.push((0, _ui_utils.removenullcharacters)(buffer.join(""))); texts.join("\n"); #copycallback(textlayermode, event) selection="document.getSelection();" focusnode, anchornode (anchornode && focusnode selection.containsnode(this.#hiddencopyelement)) (this.#getalltextinprogress || textlayermode="==" _ui_utils.textlayermode.enable_permissions) event.preventdefault(); event.stoppropagation(); return; this.#getalltextinprogress="true;" savedcursor="this.container.style.cursor;" this.container.style.cursor="wait" ; interruptcopy="ev" => this.#interruptCopyCondition = ev.key === "Escape";
  ...
  previousPage() {
    const currentPageNumber = this._currentPageNumber;
    if (currentPageNumber <= 1) { return false; } const advance="this._getPageAdvance(currentPageNumber," true) || 1; this.currentpagenumber="Math.max(currentPageNumber" - advance, 1); true; increasescale({ drawingdelay, scalefactor, steps if (!this.pdfdocument) return; let newscale="this._currentScale;" (scalefactor> 1) {
...
const DEFAULT_L10N_STRINGS = {
  ...
  document_properties_date_string: "1685883810389, ",
